...
import {
  ArrowRight, Crown, Database, List, BarChart2,
  Shield, Activity, FileText, Settings, TrendingUp,
} from 'lucide-react';
...
const sections = [
  {
    ...
    icon: List,
    items: ['جرد الأصول', 'بيانات الأصول', 'التصنيفات'],
  },
  {
    ...
    icon: Activity,
    items: ['حالة الأصول', 'التنبيهات', 'الأداء'],
  },
  {
    ...
    icon: Settings,
    items: ['خطط الصيانة', 'سجل الأعمال', 'الإجراءات'],
  },
  {
...
const mgmtLinks = [
  ...
  { href: '/dashboard/admin-gateway/assets/maintenance', icon: Settings,  label: 'الصيانة' },
  { href: '/dashboard/admin-gateway/workflow/approvals?role=supervisor&dept=assets', icon: FileText, label: 'الموافقات' },
];
...
const monitoringLinks = [
  ...
  },
  {
    href: '/dashboard/admin-gateway/assets/health',
    icon: BarChart2,
    label: 'مؤشرات صحة الأصول',
    hint: 'متابعة حالة الأصول والتنبيهات الحرجة',
  },
];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8" dir="rtl">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div>
          <Link href="/dashboard/admin-gateway/assets" className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors text-sm mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-0.5">إشراف كامل على الأصول التشغيلية والرقمية</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-2 grid grid-cols-1 md:grid-cols-3 gap-2">
          <button onClick={() => setActiveTab('sections')}
            className={`rounded-xl px-4 py-3 text-sm font-semibold transition-colors ${activeTab === 'sections' ? 'bg-cyan-500/20 border border-cyan-500/40 text-cyan-300' : 'bg-slate-900 border border-slate-800 text-slate-300 hover:bg-slate-800/60'}`}>
            أقسام الإدارة
          </button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {mgmtLinks.map(({ href, icon: Icon, label }) => (
                  <Link key={href} href={href}
                    className="group flex items-center gap-3 bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 hover:border-cyan-500/40 hover:bg-slate-800/60 transition-all">
