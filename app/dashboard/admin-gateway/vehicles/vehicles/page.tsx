...
import { Truck, Plus, Search, X, Edit2, ChevronLeft } from 'lucide-react';
...
type VStatus = 'active' | 'maintenance' | 'out_of_service' | 'reserved' | 'retired';
type VType   = 'truck' | 'pickup' | 'van' | 'bus' | 'sedan' | 'tanker' | 'crane' | 'loader' | 'other';

interface Project { id: number; name: string; }
interface Site    { id: number; name: string; project_id: number; }
interface Vehicle {
  id: number; vehicle_code: string; vehicle_name: string;
  vehicle_type: VType; plate_number?: string; make?: string; model?: string; year?: number;
  status: VStatus; assigned_project_id?: number; assigned_project_name?: string;
  assigned_site_id?: number; assigned_site_name?: string; notes?: string; created_at: string;
}

const TYPE_LABELS: Record<string, string> = {
  truck: 'شاحنة', pickup: 'بيك أب', van: 'فان', bus: 'حافلة', sedan: 'سيارة',
  tanker: 'صهريج', crane: 'رافعة', loader: 'لودر', other: 'أخرى',
};
...
const emptyForm = {
  vehicle_name: '', vehicle_type: 'pickup' as VType, vehicle_code: '',
  plate_number: '', make: '', model: '', year: '' as string | number,
  status: 'active' as VStatus, assigned_project_id: '' as string | number,
  assigned_site_id: '' as string | number, notes: '',
};
...
export default function VehiclesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (filterType)    p.set('vehicle_type',  filterType);
      if (filterProject) p.set('project_id',    filterProject);
      const r = await fetch(`${BASE}/vehicles?${p}`, { headers: { 'X-Tenant-ID': getTenantId() || '' } });
  ...
  const openEdit = (v: Vehicle) => {
    setEditing(v);
    setForm({
      vehicle_name: v.vehicle_name, vehicle_type: v.vehicle_type,
      vehicle_code: v.vehicle_code, plate_number: v.plate_number || '',
      make: v.make || '', model: v.model || '', year: v.year || '',
      status: v.status,
      assigned_project_id: v.assigned_project_id || '',
  ...
  const save = async () => {
    if (!form.vehicle_name.trim()) { setError('اسم المركبة مطلوب'); return; }
    setSaving(true); setError('');
    try {
      const url    = editing ? `${BASE}/vehicles/${editing.id}` : `${BASE}/vehicles`;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 space-y-6" dir="rtl">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-400" dir="ltr">
        ...
        <ChevronLeft className="w-4 h-4" />
        <span className="text-slate-200">المركبات</span>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 bg-slate-900/50 p-5 rounded-2xl border border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-100">سجل المركبات</h1>
            <p className="text-slate-400 text-sm">{total} مركبة مسجلة</p>
          </div>
        </div>
        <div className="flex gap-2">
          <ImportButton module="vehicles" onSuccess={load} />
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="بحث بالاسم أو الرمز أو اللوحة..."
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-500">لا توجد مركبات — أضف أول مركبة</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-800 text-slate-400 text-xs uppercase">
                <tr>
                  {['الرمز', 'الاسم', 'النوع', 'رقم اللوحة', 'الصانع / الموديل', 'المشروع', 'الموقع', 'الحالة', ''].map((h, i) => (
              ...
              <tbody className="divide-y divide-slate-800">
                {rows.map(v => (
                  <tr key={v.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3 text-blue-300 font-mono text-xs">{v.vehicle_code}</td>
                    <td className="px-4 py-3 text-slate-100 font-medium">{v.vehicle_name}</td>
                    <td className="px-4 py-3 text-slate-300">{TYPE_LABELS[v.vehicle_type] || v.vehicle_type}</td>
                    <td className="px-4 py-3 text-slate-300 font-mono text-xs">{v.plate_number || '—'}</td>
                    <td className="px-4 py-3 text-slate-400">{[v.make, v.model, v.year].filter(Boolean).join(' / ') || '—'}</td>
                    <td className="px-4 py-3 text-slate-400 text-xs">{v.assigned_project_name || '—'}</td>
                    <td className="px-4 py-3 text-slate-400 text-xs">{v.assigned_site_name || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs border ${STATUS_COLORS[v.status]}`}>
                        {STATUS_LABELS[v.status]}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => openEdit(v)} className="p-1.5 text-slate-400 hover:text-blue-400 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
            <div className="flex items-center justify-between p-5 border-b border-slate-700">
              <h2 className="text-lg font-bold text-slate-100">{editing ? 'تعديل مركبة' : 'إضافة مركبة جديدة'}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="block text-slate-300 text-sm mb-1.5">اسم المركبة <span className="text-red-400">*</span></label>
                  <input value={form.vehicle_name} onChange={e => F('vehicle_name', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 text-sm" />
                </div>

                <div>
                  <label className="block text-slate-300 text-sm mb-1.5">نوع المركبة</label>
                ...
                <div>
                  <label className="block text-slate-300 text-sm mb-1.5">رمز المركبة <span className="text-slate-500 text-xs">(اختياري — يُنشأ تلقائياً)</span></label>
                  <input value={form.vehicle_code} onChange={e => F('vehicle_code', e.target.value)}
                    placeholder="VEH-..."
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 text-sm font-mono" />
                </div>

                <div>
                  <label className="block text-slate-300 text-sm mb-1.5">رقم اللوحة</label>
                  <input value={form.plate_number} onChange={e => F('plate_number', e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 text-sm" />
                </div>

                <div>
                  <label className="block text-slate-300 text-sm mb-1.5">الصانع (Make)</label>
                ...
                <div>
                  <label className="block text-slate-300 text-sm mb-1.5">سنة الصنع</label>
            ...
            <div className="flex gap-3 p-5 border-t border-slate-700">
              <button onClick={save} disabled={saving}
                ...
                {saving ? 'جاري الحفظ...' : editing ? 'حفظ التعديلات' : 'إضافة المركبة'}
              </button>
