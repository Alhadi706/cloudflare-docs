...
import {
  ArrowRight, Crown, BookOpen, BarChart2, DollarSign,
  FileText, Activity, Settings, PieChart, TrendingUp,
} from 'lucide-react';
...
const sections = [
  {
    ...
    icon: BookOpen,
    items: ['القيود اليومية', 'التسويات', 'الاعتمادات'],
  },
  {
    ...
    icon: Settings,
    items: ['الحسابات الرئيسية', 'التصنيفات', 'الميزانية العمومية'],
  },
  {
    ...
    icon: PieChart,
    items: ['تخصيص التكاليف', 'التقارير', 'التحليل'],
  },
  {
    ...
    icon: BarChart2,
    items: ['قائمة الدخل', 'الميزانية العمومية', 'التدفقات'],
  },
];

const mgmtLinks = [
  { href: '/dashboard/admin-gateway/accounting/journal-entries',  icon: BookOpen,   label: 'القيود المحاسبية' },
  { href: '/dashboard/admin-gateway/accounting/chart-of-accounts', icon: Settings,  label: 'دليل الحسابات' },
  { href: '/dashboard/admin-gateway/accounting/cost-centers',     icon: PieChart,   label: 'مراكز التكلفة' },
  { href: '/dashboard/admin-gateway/finance/reports',             icon: BarChart2,  label: 'التقارير المالية' },
  { href: '/dashboard/admin-gateway/finance/expenses',            icon: DollarSign, label: 'المصروفات' },
  { href: '/dashboard/admin-gateway/workflow/approvals?role=finance_controller', icon: FileText, label: 'الموافقات' },
];
...
const monitoringLinks = [
  ...
  },
  {
    href: '/dashboard/admin-gateway/finance/reports',
    icon: TrendingUp,
    label: 'مؤشرات الأداء المحاسبي',
    hint: 'متابعة الإيرادات والمصروفات والتوازن المالي',
  },
];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8" dir="rtl">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div>
          <Link href="/dashboard/admin-gateway/accounting" className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors text-sm mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">لوحة مدير إدارة الشؤون المحاسبية</h1>
              <p className="text-slate-400 text-sm mt-0.5">إشراف كامل على السجلات المحاسبية والتقارير المالية</p>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'correspondence' && (
          <div>
            ...
            <InternalMailTab department="finance_manager" title="نظام المراسلات الموحد - إدارة الشؤون المحاسبية" />
          </div>
        )}
