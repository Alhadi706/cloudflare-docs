...
import {
  ShoppingCart, Package, Warehouse, ClipboardList, Truck, BarChart2, FileText, CheckCircle, Crown,
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6" dir="rtl">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Manager Card */}
        <Link href="/dashboard/procurement/manager" className="group block mb-8">
          <div className="bg-slate-900 border border-orange-500/30 rounded-2xl p-5 hover:border-orange-500/60 hover:bg-slate-800/70 transition-all duration-200">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-orange-500/20 flex items-center justify-center shrink-0">
                <Crown className="w-5 h-5 text-orange-400" />
              </div>
              <div className="flex-1">
                <h2 className="text-base font-bold text-white">مدير إدارة المشتريات والمستودعات</h2>
                <p className="text-orange-400/70 text-xs mt-0.5">لوحة المدير — الأقسام والمراسلات الإدارية</p>
              </div>
              <span className="text-orange-400 text-xs font-semibold flex items-center gap-1">فتح لوحة المدير <span className="group-hover:translate-x-[-3px] transition-transform inline-block">←</span></span>
            </div>
          </div>
        </Link>
