...
const FILE_PATH = path.join(DATA_DIR, 'credential_login_attempts.json');
...
function readAll(): CredentialLoginAttempt[] {
  ensureDir();
  if (!fs.existsSync(FILE_PATH)) return [];
  try {
    return JSON.parse(fs.readFileSync(FILE_PATH, 'utf8')) as CredentialLoginAttempt[];
  } catch {
    return [];
  }
}

function writeAll(rows: CredentialLoginAttempt[]): void {
  ensureDir();
...
export function buildCredentialAttemptKey(tenantHint: string, identity: string, ip: string): string {
  return `${tenantHint}:${identity}:${ip || 'unknown_ip'}`;
}

export function getCredentialLockState(key: string): { locked: boolean; retryAfterSec: number } {
  const rows = readAll();
...
export function registerFailedCredentialLogin(key: string): { locked: boolean; retryAfterSec: number } {
  const now = Date.now();
  const rows = readAll();
...
export function clearCredentialLoginFailures(key: string): void {
  const rows = readAll();
